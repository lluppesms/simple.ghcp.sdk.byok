...
param existingServicePlanName string = '' // if using an existing service plan
param existingServicePlanResourceGroupName string = '' // if using an existing service plan in a different resource group
...
var existingServicePlanNameEffective = empty(trim(existingServicePlanName)) || contains(existingServicePlanName, '#{') ? '' : trim(existingServicePlanName)
var existingServicePlanRgNameEffective = empty(trim(existingServicePlanResourceGroupName)) || contains(existingServicePlanResourceGroupName, '#{') ? '' : trim(existingServicePlanResourceGroupName)
